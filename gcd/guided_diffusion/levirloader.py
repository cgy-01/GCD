...
class LEVIRDataset(Dataset):
    ...
    def __init__(self, args, data_path, transform=None, mode='train', plane=False):
        self.split = "train"
        self.root_dir = r"F:\Datasets\ChangeDetection\LEVIR-CD\train"
        self.list_dir = r"F:\Datasets\ChangeDetection\LEVIR-CD"
        self.list_path = os.path.join(self.list_dir, LIST_FOLDER_NAME, self.split + '.txt')
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            state = torch.get_rng_state() # 设置随机数 确保A B L操作一致
