...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb):
        for layer in self:
            if isinstance(layer, TimestepBlock):  # 无缝的实现方法 检查当前层是否是TimestepBlock的实例 如果是就加入时间序列emb
                x = layer(x, emb)
            else:
                x = layer(x)
        return x
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        assert x.shape[1] == self.channels  # 确保输入的x通道数和channels相同
        if self.dims == 3:  # 3D信号需要转化为2D
            x = F.interpolate(
                x, (x.shape[2], x.shape[3] * 2, x.shape[4] * 2), mode="nearest"
            )
        else:
            x = F.interpolate(x, scale_factor=2, mode="nearest")  # 最邻近插值
        if self.use_conv:
            x = self.conv(x)
        return x
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if self.out_channels == channels:
            self.skip_connection = nn.Identity()  # 不适用卷积就直接空
        elif use_conv:
            self.skip_connection = conv_nd(
                dims, channels, self.out_channels, 3, padding=1
            )  # 卷积则将sc定义为一个卷积层
        else:
            self.skip_connection = conv_nd(dims, channels, self.out_channels, 1)  # 入出纬度不同但不用卷积 则为1*1卷积
    ...
    def _forward(self, x, emb):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]  # in_layers[-1]是最后一层卷积层
            h = in_rest(x)
            h = self.h_upd(h)
            x = self.x_upd(x)
            h = in_conv(h)
...
def count_flops_attn(model, _x, y):
    # 这个函数用于计算注意力操作中的浮点操作数（FLOPs），并将结果添加到模型的总操作数中。这可以用来评估模型的计算复杂度。
    """
...
class FFParser(nn.Module):
    def __init__(self, dim, h=128, w=65):
        ...
        self.complex_weight = nn.Parameter(torch.randn(32*32*17, 2, dtype=torch.float32) * 0.02)
        self.w = w
        self.h = h
    ...
    def forward(self, x, spatial_size=None):
        ...
        # print("after x = x.to(torch.float32):{}".format(x.shape))
        x = torch.fft.rfft2(x, dim=(2, 3), norm='ortho')
        x_size = x.shape
        a,b,c,d = x_size[0],x_size[1],x_size[2],x_size[3]

        # print("after x = torch.fft.rfft2(x, dim=(2, 3), norm='ortho'):{}".format(x.shape))
        weight = torch.view_as_complex(self.complex_weight)
        weight = weight[0:a*b*c*d]
        weight = weight.view(x_size)

        # print("after weight = weight.view(1, 32, 32, 17):{}".format(weight.shape))
        x = x * weight
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.AB_Concator = cator(3, 2)  # 变化图片通道数3 变化时相2
        # net, net_name = SiamUnet_diff(3, 2)
        self._feature_size = model_channels
        input_block_chans = [model_channels]
        ch = model_channels
        ds = 1
        channel_mult = (1,1,2,2,4,4)  # 其中每个元素对应于 UNet 中的一个级别（或者说每个下采样/上采样阶段），而其中的值表示该级别的通道数相对于基础通道数的倍增量。
        for level, mult in enumerate(channel_mult):
            ...
            for _ in range(num_res_blocks):
                ...
                '''
                在代码的后续部分，如果当前级别的下采样率 ds 在 attention_resolutions 集合中，
                就会在当前级别的 layers 中添加一个注意力块（Attention Block），以便在这个下采样率处进行注意力操作。
                '''
    ...
    def forward(self, x, timesteps, y=None):
        ...
        A_img = x[:, 0:3, ...] # channel = 3
        B_img = x[:, 3:6, ...] # channel = 3
        mask_noisy = x[:, 6:7, ...] # channel = 1


        ##########################################################################################
        # there's problem-Whatis AB_Concator from 3to2channel
        ##########################################################################################

        diff_img = self.AB_Concator(A_img,B_img) # 输出channel=2
        # diff_img = self.con2to3(diff_img)  # diff_img channel = 2 to 3
        # diff_img = th.cat((diff_img,mask_noisy),dim=1)  # channel = 4
        A_img = th.cat((A_img,mask_noisy),dim=1)  # channel = 4
        B_img = th.cat((B_img,mask_noisy),dim=1)  # channel = 4
...
class ConvDropoutNormNonlin(nn.Module):
    ...
    def __init__(self, input_channels, output_channels,
        ...
        #################################################################将三通道转为双通道
        self.conv_error = nn.Conv2d(in_channels=3, out_channels=2, kernel_size=1)
        ###########################################################################################################################################
        # print(self.conv_error.requires_grad)
