...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x,x2, hs):
        ...
        # skips2 = [] # x_diff = x2 - x ####################################################################
        diff = []   ######################################
        seg_outputs = [] # x  (1, 3, 256, 256)
        for d in range(len(self.conv_blocks_context) - 1):
            print(self.conv_blocks_context[d]) # (conv): Conv2d(3, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)) (dropout) (instnorm) (lrelu) 
            x = self.conv_blocks_context[d](x) #  (1, 32, 256, 256)   (1, 64, 128, 128)
            x2 = self.conv_blocks_context[d](x2)
            # skips.append(x)
            # skips2.append(x2)
            diff.append(x2-x) #  (1, 32, 256, 256)   (1, 64, 128, 128)
            if not self.convolutional_pooling:
                print(self.td[d]) # MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=0, dilation=1, ceil_mode=False)
                x = self.td[d](x) # (1, 32, 128, 128) (1, 64, 64, 64)
                x2 = self.td[d](x2)
                x_diff = x2-x # (1, 32, 128, 128) (1, 64, 64, 64)
            if hs:
                h = hs.pop(0) #  (1, 128, 128, 128) (1, 128, 64, 64)
                # h2 = hs2.pop(0)
                # ddims = h.size(1)
                h = self.conv_trans_blocks_a[d](h) # (1, 32, 128, 128)  (1, 64, 64, 64) 
                print(self.conv_trans_blocks_a[d]) # MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=0, dilation=1, ceil_mode=False)
                # h2 = self.conv_trans_blocks_a[d](h2)
                h = self.ffparser[d](h)  # (1, 32, 128, 128)
                print(self.ffparser[d])  #  FFParser()
                # h2 = self.ffparser[d](h2)
                ha = self.conv_trans_blocks_b[d](h) # (1, 1, 128, 128) 
                print(self.conv_trans_blocks_b[d]) # Conv2d(32, 1, kernel_size=(1, 1), stride=(1, 1))
                # ha2 = self.conv_trans_blocks_b[d](h2)
                hb = th.mean(h,(2,3)) #  (1, 32, 1, 1)
                # hb2 = th.mean(h2,(2,3))
                hb = hb[:,:,None,None]
                # hb2 = hb2[:,:,None,None]
                x_diff = x_diff * ha * hb
        ...
        # x_diff change code
        x= x_diff #  (1, 480, 8, 8)
        x = self.conv_blocks_context[-1](x) # (1, 480, 8, 8)
        emb = conv_nd(2, x.size(1), 512, 1).to(device = x.device)(x) # (1, 512, 8, 8)

        for u in range(len(self.tu)):
            print(self.tu[u]) #  (1, 480, 8, 8)  (1, 256, 16, 16)  (1, 128, 32, 32) (1, 64, 64, 64)  (1, 32, 128, 128)hwUpsample()
            x = self.tu[u](x) #  (1, 480, 16, 16)  (1, 256, 32, 32) (1, 128, 64, 64) (1, 64, 128, 128) (1, 32, 256, 256)
            # x = th.cat((x, skips[-(u + 1)]), dim=1)
            x = th.cat((x, diff[-(u + 1)]), dim=1) # (1, 960, 16, 16)  (1, 512, 32, 32) (1, 256, 64, 64) (1, 128, 128, 128) (1, 64, 256, 256)
            print(self.conv_blocks_localization[u]) 
            x = self.conv_blocks_localization[u](x) # (1, 256, 16, 16)  (1, 128, 32, 32) (1, 64, 64, 64)  (1, 32, 128, 128)  (1, 32, 256, 256)
            if self._deep_supervision:
                
                seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x)))
        if not seg_outputs:
            seg_111 = self.final_nonlin(self.seg_outputs[u](x))
